    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-5 gap-px overflow-hidden rounded-lg border border-border">
        {zones.map((zone) => {
          ...
          return (
            <div key={zone.key} className={`flex flex-col gap-2 p-3 ${zone.bg}`}>
              <span className={`text-[9px] font-bold uppercase tracking-[1.5px] ${zone.color}`}>
                {zone.label}
              </span>
              <div className="flex flex-col gap-1.5">
                {zoneSources.map((source) => (
                  <a
                    ...
                    className="flex items-center gap-2 rounded px-1.5 py-1 text-xs text-foreground transition-colors hover:bg-elevated/50"
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  >
                    {source.logoUrl ? (
                      <img src={source.logoUrl} alt="" className="size-5 shrink-0 rounded-full object-cover" />
                    ) : (
