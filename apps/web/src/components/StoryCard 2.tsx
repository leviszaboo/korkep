...
export function StoryCard({ story, featured = false, compact = false, fill = false }: StoryCardProps) {
  if (featured) return <FeaturedCard story={story} />;
  if (compact) return <CompactCard story={story} fill={fill} />;
...
function FeaturedCard({ story }: { story: Story }) {
  ...
  return (
    <Link href={`/stories/${story.id}`} className="group block">
      <article className="overflow-hidden border-b-2 border-border p-6 transition-all sm:p-8">
...
function CompactCard({ story, fill = false }: { story: Story; fill?: boolean }) {
  const widelyCovered = story.sourceCount >= 8;
