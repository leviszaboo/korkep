...
const activity = buildActivity('manual', 'resummarize', 'summarize');
...
async function resummarizeOneStory(storyId: number) {
  ...
  const result = storyArticles.length >= 2
    ? await generateStoryTitleAndSummary(storyArticles.map((a, i) => ({ id: i, title: a.title, summary: a.summary })), activity)
    : null;
